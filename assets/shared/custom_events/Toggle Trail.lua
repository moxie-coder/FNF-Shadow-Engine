...
function createTrailFrame(tag)
    ...
    if tag == 'BF' then
        ...
        if trailEnabledBF then
            ...
            local newPath = isMultiAtlas and (imageFile:match("(.+)/[^/]+$") or imageFile) .. '/' .. animName or
                                imageFile
            image = newPath
    ...
    if num - trailLength + 1 >= 0 then
        for i = (num - trailLength + 1), (num - 1) do
            setProperty('psychicTrail' .. tag .. i .. '.alpha',
                getProperty('psychicTrail' .. tag .. i .. '.alpha') - (0.6 / (trailLength - 1)))
        end
    end
